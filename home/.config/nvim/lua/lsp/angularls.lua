...
local function get_angular_core_version()
  ...
  local function read_core_version(package_json_path)
  ...
  local v = read_core_version(root_pkg)
  if v then
    return v
  end

  local patterns = {
    project_root .. '/packages/*/package.json',
    project_root .. '/projects/*/package.json',
  }

  for _, pattern in ipairs(patterns) do
    local matches = vim.fn.glob(pattern, true, true) or {}
    for _, pkg in ipairs(matches) do
      local mv = read_core_version(pkg)
      if mv then
        return mv
      end
    end
